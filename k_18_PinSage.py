...
# Varijanta: PinSage – agregacija vođena random walk (težine ivica); 2 sloja propagacije. 

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data. 
PinSage = importance pooling + GNN; ovde P = D^{-1}A, 2 sloja H = P^2 @ H0
"""
...
PINSAGE_LAYERS = 2 # broj slojeva propagacije
...
def compute_pinsage_embeddings(M, k=EMBED_DIM):
    ...
    H = vecs[:, 1:1 + n_eig].astype(float)
    if n_eig < k:
        H = np.hstack([H, np.tile(H[:, -1:], (1, k - n_eig))])

    for _ in range(PINSAGE_LAYERS):
        H = P @ H
...
def main():
    ...
    emb = compute_pinsage_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (PinSage + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (PinSage + kvantna regresija, deterministički, strukturno):
    (8, 11, x, y, z, 33, 34)

    Score: 0.5001153083135611
    """
...
matrica prelaza random walk, ne red-normalizovana A).

H0 = prvih k sopstvenih vektora Laplaciana.

2 sloja propagacije: H = P @ (P @ H0) = P^2 @ H0 
(agregacija u duhu PinSage, 
bez nasumičnog uzorkovanja, deterministički).
